...
//! フォールバックで求める。
//!
//! 注記: 本モジュールが依存する `tags` / `work_tags` テーブルは、issue #7 着手時点では
//! issue #2（Tagエンティティと Work-Tag 多対多リレーション）が未マージだったため、
//! `crates/topcoat-db/migrations` 配下に issue #7 が必要とする範囲に限定した暫定スキーマ
//! として先行作成していた（詳細は各マイグレーションのコメントを参照）。issue #2 マージ後は
//! カラム構成の変更なく正式実装として引き継がれている（Diesel モデル・クエリ関数は
//! `crate::models::tag`・`crate::queries::tag` を参照）。
